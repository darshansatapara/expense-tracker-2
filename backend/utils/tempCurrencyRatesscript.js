...
// Get all dates in the current month from the 1st to today
const getAllDatesInCurrentMonth = () => {
  const now = new Date();
  const year = now.getFullYear();
  const currentMonth = now.getMonth();

  const firstDay = new Date(year, currentMonth, 1); // Start from 1st of current month
  const today = new Date(); // Up to today
...
// Store exchange rates for the current month
const storeCurrentMonthRates = async (adminDbConnection) => {
  const today = new Date();
  const year = today.getFullYear();
  ...
  if (!currencyRate) {
    currencyRate = new CurrencyRateModel({
      ...
      months: [
        {
          startMonth: formatDate(new Date(year, currentMonth, 1)),
          endMonth: formatDate(new Date(year, currentMonth + 1, 0)),
          days: [],
        },
      ],
    });
  ...
  let monthData = currencyRate.months.find(
    (m) => m.startMonth === formatDate(new Date(year, currentMonth, 1))
  );
  ...
  const existingDates = monthData.days.map((d) => d.date);
  const allDates = getAllDatesInCurrentMonth(); // Dates from 1st to today
  const newDates = allDates.filter((d) => !existingDates.includes(d));

  if (newDates.length === 0) {
    console.log(
      "✅ All dates for current month up to today already have exchange rates."
    );
    return;
  }
...
const startFetchingRates = async () => {
  ...
  await storeCurrentMonthRates(adminDbConnection);
};
